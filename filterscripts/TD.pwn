...
PlayerTextDraw function add by adri1. (Edit 1) *25/10/12*
SetSelectable function add by adri1 (Edit 2) *26/10/12*
2D TextDraws function add by adri1 (Edit 3) *11/01/13* [SA-MP 0.3x only]

--- Dev By Kar -- (05/06/2013)

Fixed some "2D TextDraws" issues by ardi1 - Kar
Made creating sprites and Model previews easier, by auto enabling boxes and making a default preview / sprite - Kar
    ...
    switch(dialogid)
    {
        ...
        case 1591: // Change textdraw's font
        {
            if(response)
            {
                ...
                if(listitem == 4)
                {
                    SendClientMessage(playerid, -1, "Important: Box Has Been Auto Enabled - Background Removed!");
                    SendClientMessage(playerid, -1, "Change the box size to change TD size - Default Up Arrow Created (ld_beat:up).");
                ...
                {
                    SetPVarInt(playerid, "Use2DTD", 1);
                    SendClientMessage(playerid, -1, "Important: Box Has Been Auto Enabled!");
                    tData[pData[playerid][P_CurrentTextdraw]][T_UseBox] = 1;
                    tData[pData[playerid][P_CurrentTextdraw]][T_TextSizeX] = 30.0;
    				tData[pData[playerid][P_CurrentTextdraw]][T_TextSizeY] = 30.0;
					UpdateTextdraw(pData[playerid][P_CurrentTextdraw]);
        ...
        case 1606: // Change textdraw's selectable
        {
            if(response)
            {
                ...
                new string[128];
                format(string, sizeof(string), "Textdraw #%d's is NOW selectable by mouse click. (selection has been turned ON)", pData[playerid][P_CurrentTextdraw]);
                SendClientMessage(playerid, MSG_COLOR, string);
                ShowTextDrawDialog(playerid, 5);
            }
            else
            {
                ...
                new string[128];
                format(string, sizeof(string), "Textdraw #%d's is NOT selectable by mouse click (selection has been turned OFF).", pData[playerid][P_CurrentTextdraw]);
                SendClientMessage(playerid, MSG_COLOR, string);
                ShowTextDrawDialog(playerid, 5);
            }
        }
