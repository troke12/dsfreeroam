...
public onSocketReceiveData(Socket:id, remote_clientid, data[], data_len)
{
	...
	sscanf(data, "p<:>s[128]s[128]s[128]", splitdata[0], splitdata[1], splitdata[2]);

	if(splitdata[0][0] == 'i' && strlen(splitdata[2])) sscanf(splitdata[2], "p<:>s[128]s[128]s[128]", splitdata[0], splitdata[1], splitdata[2]);

	if(splitdata[0][0] == 'i')
	{
		new iID;

		sscanf(data, "p<:>s[128]us[128]", splitdata[0], iID, splitdata[2]);
	...
	else if(strcmp(splitdata[0], "ban", true) == 0)
	{
		...
		else
		{
			...
			sscanf(data, "p<:>s[128]iis[50]", splitdata[0], banID, banTime, banReason);
	...
	else if(strcmp(splitdata[0], "skin", true) == 0)
	{
		...
		else
		{
			...
			sscanf(data, "p<:>s[128]ui", splitdata[0], iID, skin);
	...
	else if(strcmp(splitdata[0], "health", true) == 0)
	{
		...
		else
		{
			...
			sscanf(data, "p<:>s[128]uf", splitdata[0], iID, health);
	...
	else if(strcmp(splitdata[0], "armour", true) == 0)
	{
		...
		else
		{
			...
			sscanf(data, "p<:>s[128]uf", splitdata[0], iID, armour);
	...
	else if(strcmp(splitdata[0], "score", true) == 0)
	{
		...
		else
		{
			...
			sscanf(data, "p<:>s[128]ui", splitdata[0], iID, score);
	...
	else if(strcmp(splitdata[0], "weapon", true) == 0)
	{
		...
		else
		{
			...
			sscanf(data, "p<:>s[128]ui", splitdata[0], iID, weapon);
	...
	else if(strcmp(splitdata[0], "interior", true) == 0)
	{
		...
		else
		{
			...
			sscanf(data, "p<:>s[128]ui", splitdata[0], iID, interior);
	...
	else if(strcmp(splitdata[0], "world", true) == 0)
	{
		...
		else
		{
			...
			sscanf(data, "p<:>s[128]ui", splitdata[0], iID, world);
	...
	else if(strcmp(splitdata[0], "vhealth", true) == 0)
	{
		...
		else
		{
			...
			sscanf(data, "p<:>s[128]uf", splitdata[0], iID, vhealth);
	...
	else if(strcmp(splitdata[0], "spvar", true) == 0)
	{
		if(iLevel[remote_clientid] < PVAR_LEVEL) sendToClient(remote_clientid, "spop:You are not a high enough level!");
		else
		{
			new iID;

			sscanf(data, "p<:>s[128]us[128]s[128]", splitdata[0], iID, splitdata[1], splitdata[2]);

			if(iID == INVALID_PLAYER_ID) sendToClient(remote_clientid, "spop:The player you specified was not found!");
			else
			{
				if(IsNumeric(splitdata[2])) SetPVarInt(iID, splitdata[1], strval(splitdata[2]));
				else SetPVarString(iID, splitdata[1], splitdata[2]);

				format(gQuery, sizeof(gQuery), "spop:The PVar \"%s\" has been changed for %s", splitdata[1], strNames[iID]);
				sendToClient(remote_clientid, gQuery);
			}
		}
	}
	...
	else if(strcmp(splitdata[0], "aset", true) == 0)
	{
		...
		else
		{
			...
			sscanf(splitdata[2], "p<:>s[24]i", tmpName, tmpLevel);
	...
	else if(strcmp(splitdata[0], "anew", true) == 0)
	{
		...
		else
		{
			...
			sscanf(data, "p<:>s[128]s[128]is[24]", splitdata[0], splitdata[1], tmpLevel, tmpName);
	...
	else if(strcmp(splitdata[0], "setb", true) == 0)
	{
		...
		else
		{
			...
			sscanf(data, "p<:>s[128]is[24]s[16]s[24]iis[50]", splitdata[0], banID, banName, banIP, banAdmin, banMinutes, banStamp, banReason);
...
// Thanks Y_Less for this function
