...
{
    print("\n>> GarHouse v1.1.3 By [03]Garsino Loaded <<\n");
    LoadHouses(); // Load houses
    return 1;
}
public OnFilterScriptExit()
{
	print("\n>> GarHouse v1.1.3 By [03]Garsino Unloaded <<\n");
	    ...
	    Loop(h, MAX_HOUSES)
		{
		    ...
		    if(checkpointid == HouseCPOut[h])
		    {
		        ...
		        if(strcmp(GetHouseOwner(h), pNick(playerid), CASE_SENSETIVE) && strcmp(GetHouseOwner(h), INVALID_HOWNER_NAME, CASE_SENSETIVE))
		        {
					...
					{
		        		ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "House Name: %s\nHouse Owner: %s\nHouse Value: $%d\nHouse ID: %d", GetHouseName(h), GetHouseOwner(h), GetHouseValue(h), h);
					}
					if(strcmp(dini_Get(file, "HousePassword"), "INVALID_HOUSE_PASSWORD", CASE_SENSETIVE))
					{
	                	...
	                	ShowPlayerDialog(playerid, HOUSEMENU+60, DIALOG_STYLE_INPUT, "GarHouse v1.1.3 By [03]Garsino - House Info", string, "Enter", "Close");
					}
		        }
		        if(!strcmp(GetHouseOwner(h), INVALID_HOWNER_NAME, CASE_SENSETIVE) && dini_Int(file, "HouseValue") > 0)
				{
					...
					ShowPlayerDialog(playerid, HOUSEMENU+4, DIALOG_STYLE_MSGBOX, "GarHouse v1.1.3 By [03]Garsino - House Buying", string, "Buy", "Cancel");
				}
		    }
		    if(checkpointid == HouseCPInt[h])
		    {
		        if(GetPVarInt(playerid, "HousePreview") == 1)
		        {
					...
					ShowPlayerDialog(playerid, HOUSEMENU+17, DIALOG_STYLE_MSGBOX, "GarHouse v1.1.3 By [03]Garsino - House Interior Upgrade", string, "Buy", "Cancel");
		        }
		        else
		        {
	...
	if(dialogid == HOUSEMENU && response)
	{
	    switch(listitem)
		{
		    case 0:
		    {
				...
				ShowPlayerDialog(playerid, HOUSEMENU+3, DIALOG_STYLE_MSGBOX, "GarHouse v1.1.3 By [03]Garsino - House Sale", string, "Sell", "Cancel");
		    }
		    case 1:
			{
				#if defined GH_USE_WEAPONSTORAGE
					ShowPlayerDialog(playerid, HOUSEMENU+18, DIALOG_STYLE_LIST, "GarHouse v1.1.3 By [03]Garsino - House Storage", "Money Storage\nWeapon Storage", "Select", "Cancel");
				#endif
				#if !defined GH_USE_WEAPONSTORAGE
					ShowPlayerDialog(playerid, HOUSEMENU+10, DIALOG_STYLE_LIST, "GarHouse v1.1.3 By [03]Garsino - House Storage", "Deposit Money\nWithdraw Money\nCheck Balance", "Select", "Cancel");
				#endif
			}
			case 2: ShowPlayerDialog(playerid, HOUSEMENU+14, DIALOG_STYLE_INPUT, "GarHouse v1.1.3 By [03]Garsino - House Name", "Type In The New House Name Below:\n\nPress 'Cancel' To Cancel", "Done", "Cancel");
		    case 3: ShowPlayerDialog(playerid, HOUSEMENU+13, DIALOG_STYLE_INPUT, "GarHouse v1.1.3 By [03]Garsino - House Password", "Type In The New House Password Below:\nLeave The Box Empty If You Want To Keep Your Current House Password.\nPress 'Remove' To Remove The House Password.", "Done", "Remove");
		    case 4: ShowPlayerDialog(playerid, HOUSEMENU+16, DIALOG_STYLE_LIST, "GarHouse v1.1.3 By [03]Garsino - Select House Interior Option", "Preview House Interior\nBuy House Interior", "Select", "Cancel");
		}
		return 1;
	}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
	if(dialogid == HOUSEMENU+4)
	{
		if(response)
		{
			...
			if(GetOwnedHouses(playerid) >= MAX_HOUSES_OWNED) { ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "You Already Own %d House%s. Sell One Of Your Others Before Buying A New.", MAX_HOUSES_OWNED, AddS(MAX_HOUSES_OWNED)); return 0; }
			...
			if(GetHouseValue(h) > GetPlayerMoney(playerid)) { ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "You Can Not Afford To Buy This House.\nHouse Value: $%d.\nYou Have: $%d.\nYou Need: $%d.", GetHouseValue(h), GetPlayerMoney(playerid), (GetHouseValue(h) - GetPlayerMoney(playerid))); return 0; }
			else
			{
				...
				dini_IntSet(file, "HouseStorage", 0);
				ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "You've Successfully Bought This House For $%d!", GetHouseValue(h));
				Loop(h2, MAX_HOUSES)
				{
					if(IsHouseInRangeOfHouse(h, h2, RANGE_BETWEEN_HOUSES) && h2 != h)
					{
		...
		if(response)
		{
			...
			else
			{
			    if(strlen(inputtext) >= 1)
			    {
					...
					ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "You've Successfully Set The House Password To \"%s\"!", inputtext);
                    #if defined GH_DEBUGGING
		    ...
		    switch(GetPVarInt(playerid, "HouseIntUpgradeMod"))
		    {
				...
				case 2:
				{
					...
					if(GetPVarInt(playerid, "HousePrevValue") <= GetPlayerMoney(playerid))
					{
						...
						ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "You've Bought The House Interior %s For $%d.", string, GetPVarInt(playerid, "HousePrevValue"));
                        #if defined GH_DEBUGGING
...
//------------------------------------------------------------------------------
	if(dialogid == HOUSEMENU+16 && response)
	{
		...
		ShowPlayerDialog(playerid, HOUSEMENU+15, DIALOG_STYLE_LIST, "GarHouse v1.1.3 By [03]Garsino - House Interior", string, "Buy", "Cancel");
		return 1;
	}
//------------------------------------------------------------------------------
//                       House Interior Upgrade
    	...
    	if(listitem == 0) // Deposit
	   	{
     		format(string, sizeof(string), "You Have $%d In Your House Storage.\n\nType In The Amount You Want To Deposit Below:", tmp);
  			ShowPlayerDialog(playerid, HOUSEMENU+11, DIALOG_STYLE_INPUT, "GarHouse v1.1.3 By [03]Garsino - House Storage", string, "Deposit", "Cancel");
	    }
	    if(listitem == 1) // Withdraw
	    {
     		format(string, sizeof(string), "You Have $%d In Your House Storage.\n\nType In The Amount You Want To Withdraw Below:", tmp);
       		ShowPlayerDialog(playerid, HOUSEMENU+12, DIALOG_STYLE_INPUT, "GarHouse v1.1.3 By [03]Garsino - House Storage", string, "Withdraw", "Cancel");
    	}
	    if(listitem == 2) // Check Balance
	    {
     		ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "You Have $%d In Your House Storage.", tmp);
            #if defined GH_DEBUGGING
...
//------------------------------------------------------------------------------
	if(dialogid == HOUSEMENU+11 && response)
	{
		...
		else
		{
			...
			ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "You've Successfully Deposited $%d Into Your House Storage.\nCurrent Balance: $%d", amount, dini_Int(file, "HouseStorage"));
            #if defined GH_DEBUGGING
...
//------------------------------------------------------------------------------
	if(dialogid == HOUSEMENU+12 && response)
	{
		...
		else
		{
			dini_IntSet(file, "HouseStorage", (dini_Int(file, "HouseStorage") - amount));
			GivePlayerMoney(playerid, amount);
			ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "You've Successfully Withdrawn $%d From Your House Storage.\nCurrent Balance: $%d", amount, dini_Int(file, "HouseStorage"));
            #if defined GH_DEBUGGING
...
//------------------------------------------------------------------------------
	if(dialogid == HOUSEMENU+18 && response)
	{
		switch(listitem)
		{
		    case 0: ShowPlayerDialog(playerid, HOUSEMENU+10, DIALOG_STYLE_LIST, "GarHouse v1.1.3 By [03]Garsino - House Storage", "Deposit Money\nWithdraw Money\nCheck Balance", "Select", "Cancel");
		    case 1: ShowPlayerDialog(playerid, HOUSEMENU+19, DIALOG_STYLE_LIST, "GarHouse v1.1.3 By [03]Garsino - House Storage", "Store Your Current Weapons\nRecieve House Storage Weapons", "Select", "Cancel");
		}
	}
//------------------------------------------------------------------------------
//                          Weapon Storage
	...
	if(dialogid == HOUSEMENU+19 && response)
	{
		...
		switch(listitem)
		{
		    case 0: // Store weapons
		    {
				...
				if(tmpcount >= 1)
				{
					ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "You've Successfully Stored %d Weapon%s In Your House Storage.", tmpcount, AddS(tmpcount));
				}
				if(tmpcount == 0)
				{
					ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "You Do Not Have Any Weapons To Store.", tmpcount);
				}
	            #if defined GH_DEBUGGING
			...
			case 1: // Recieve Weapons
			{
				...
				if(tmpcount >= 1)
				{
					ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "You've Successfully Recieved %d Weapon%s From Your House Storage.", tmpcount, AddS(tmpcount));
				}
				if(tmpcount == 0)
				{
					ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "You Do Not Have Any Weapons In Your House Storage.", tmpcount); // I had to add tmpcount or it gave an error ^_^
				}
				#if defined GH_DEBUGGING
					printf("[GarHouse Debug] (OnDialogResponse) - %s (%d) Stored %d Weapon%s In The House Storage For House ID %d.", pNick(playerid), playerid, tmpcount, AddS(tmpcount), h);
				#endif
...
//------------------------------------------------------------------------------
	if(dialogid == HOUSEMENU+60)
	{
		if(response)
		{
			...
			{
				ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "You've Failed To Enter %s's House Using The Password \"%s\".", GetHouseOwner(GetPVarInt(playerid, "LastHouseCP")), inputtext);
			...
			{
				ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "You've Successfully Entered %s's House Using The Password \"%s\"!", GetHouseOwner(GetPVarInt(playerid, "LastHouseCP")), inputtext);
				SetPVarInt(playerid, "IsInHouse", 1);
...
dcmd_housemenu(playerid, params[])
{
	...
	{
		ShowPlayerDialog(playerid, HOUSEMENU, DIALOG_STYLE_LIST, "GarHouse v1.1.3 By [03]Garsino - House Menu", "Sell House\nHouse Storage\nSet House Name\nSet House Password\nBuy/Preview House Interior", "Select", "Cancel");
	}
	return 1;
}
//==============================================================================
...
dcmd_createhouse(playerid, params[])
{
	...
	if(h < 0)
	{
		ShowInfoBox(playerid, "GarHouse v1.1.3 By [03]Garsino - Information", "Sorry, But There Are Already %d Houses Created.\nDelete One Of The Current Ones Or Increase The Limit In The Script.", MAX_HOUSES);
	}
...
dcmd_ghcmds(playerid, params[])
{
	...
	{
	    return ShowPlayerDialog(playerid, HOUSEMENU-1, DIALOG_STYLE_MSGBOX, "GarHouse v1.1.3 By [03]Garsino - Commands", "/changeallprices\n/removeallhcars\n/sellallhouses\n/changeprice\n/changespawn\n/removehcar\n/sellhouse\n/housemenu\n/gotohouse\n/addhcar\n/ghcmds", "Ok", "Close");
    }
}
//##############################################################################
// 								      Publics
