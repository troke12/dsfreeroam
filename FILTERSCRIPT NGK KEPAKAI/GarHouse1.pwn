...
#define COLOUR_GLOBAL 					0x00FFFFFF
#define COLOUR_SYSTEM 		   			0xB60000FF
#define COLOUR_BLUE 					0x00FFFFFF
#define COLOUR_YELLOW 					0xFFFF2AFF
#define COLOUR_GREEN 					0x00BC00FF
//==============================================================================
//                              Configuration
//==============================================================================
#define HOUSEMENU 						25000 // Dialog ID
...
#define INVALID_CMD_MSG "Invalid Command. Use /cmds For A List Of Commands."
new HouseCPOut[MAX_HOUSES], HouseCPInt[MAX_HOUSES], HouseMIcon[MAX_HOUSES], Text3D:HouseLabel[MAX_HOUSES];
...
{
    print("\n>> GarHouse v1.0 By [03]Garsino Loaded <<\n");
    LoadHouses(); // Load houses
    return 1;
}
public OnFilterScriptExit()
{
	print("\n>> GarHouse v1.0 By [03]Garsino Unloaded <<\n");
	    ...
	    Loop(h, MAX_HOUSES)
		{
		    ...
		    if(checkpointid == HouseCPOut[h])
		    {
		        ...
		        if(strcmp(GetHouseOwner(h), pNick(playerid), CASE_SENSETIVE) && strcmp(GetHouseOwner(h), INVALID_HOWNER_NAME, CASE_SENSETIVE))
		        {
					...
					{
		        		ShowInfoBox(playerid, "GarHouse v1.0 By [03]Garsino - Information", "House Name: %s\nHouse Owner: %s\nHouse Value: $%d\nHouse ID: %d", GetHouseName(h), GetHouseOwner(h), GetHouseValue(h), h);
					}
					if(strcmp(dini_Get(file, "HousePassword"), "INVALID_HOUSE_PASSWORD", CASE_SENSETIVE))
					{
	                	...
	                	ShowPlayerDialog(playerid, HOUSEMENU+60, DIALOG_STYLE_INPUT, "GarHouse v1.0 By [03]Garsino - House Info", string, "Enter", "Close");
					}
		        }
		        if(!strcmp(GetHouseOwner(h), INVALID_HOWNER_NAME, CASE_SENSETIVE) && dini_Int(file, "HouseValue") > 0)
				{
					...
					ShowPlayerDialog(playerid, HOUSEMENU+4, DIALOG_STYLE_MSGBOX, "GarHouse v1.0 By [03]Garsino - House Buying", string, "Buy", "Cancel");
				}
		    }
		    if(checkpointid == HouseCPInt[h])
		    {
		        if(GetPVarInt(playerid, "HousePreview") == 1)
		        {
					...
					ShowPlayerDialog(playerid, HOUSEMENU+17, DIALOG_STYLE_MSGBOX, "GarHouse v1.0 By [03]Garsino - House Interior Upgrade", string, "Buy", "Cancel");
		        }
		        else
		        {
	...
	if(dialogid == HOUSEMENU && response)
	{
	    switch(listitem)
		{
		    case 0:
		    {
				...
				ShowPlayerDialog(playerid, HOUSEMENU+3, DIALOG_STYLE_MSGBOX, "GarHouse v1.0 By [03]Garsino - House Sale", string, "Sell", "Cancel");
		    }
		    case 1: ShowPlayerDialog(playerid, HOUSEMENU+10, DIALOG_STYLE_LIST, "GarHouse v1.0 By [03]Garsino - House Storage", "Deposit Money\nWithdraw Money\nCheck Balance", "Select", "Cancel");
		    case 2: ShowPlayerDialog(playerid, HOUSEMENU+14, DIALOG_STYLE_INPUT, "GarHouse v1.0 By [03]Garsino - House Name", "Type In The New House Name Below:\n\nPress 'Cancel' To Cancel", "Done", "Cancel");
		    case 3: ShowPlayerDialog(playerid, HOUSEMENU+13, DIALOG_STYLE_INPUT, "GarHouse v1.0 By [03]Garsino - House Password", "Type In The New House Password Below:\nLeave The Box Empty If You Want To Keep Your Current House Password.\nPress 'Remove' To Remove The House Password.", "Done", "Remove");
		    case 4: ShowPlayerDialog(playerid, HOUSEMENU+16, DIALOG_STYLE_LIST, "GarHouse v1.0 By [03]Garsino - Select House Interior Option", "Preview House Interior\nBuy/Preview House Interior", "Select", "Cancel");
		}
		return 1;
	}
//------------------------------------------------------------------------------
		...
		else
		{
			...
			Loop(h2, MAX_HOUSES)
			{
				if(IsHouseInRangeOfHouse(h, h2, RANGE_BETWEEN_HOUSES) && h2 != h)
				{
					...
					dini_IntSet(file2, "HouseValue", (dini_Int(file2, "HouseValue") - ReturnProcent(GetHouseValue(h), HOUSE_SELLING_PROCENT2)));
                    UpdateHouseText(h2);
				}
			}
			DestroyDynamicMapIcon(HouseMIcon[h]);
			UpdateHouseText(h);
			HouseMIcon[h] = CreateDynamicMapIcon(dini_Float(file, "CPOutX"), dini_Float(file, "CPOutY"), dini_Float(file, "CPOutZ"), 31, -1, dini_Int(file, "SpawnWorld"), dini_Int(file, "SpawnInterior"), -1, 50.0);
            #if defined GH_DEBUGGING
  				printf("[GarHouse Debug] (OnDialogResponse) - %s (%d) Sold Their House For $%d (House Storage: $%d). [House ID: %d]", pNick(playerid), playerid, tmp, h);
	    	#endif
		}
		return 1;
	}
...
//------------------------------------------------------------------------------
	if(dialogid == HOUSEMENU+4)
	{
		if(response)
		{
			...
			if(GetOwnedHouses(playerid) >= MAX_HOUSES_OWNED) ShowInfoBox(playerid, "GarHouse v1.0 By [03]Garsino - Information", "You Already Own %d Houses. Sell One Of Your Others Before Buying A New.", MAX_HOUSES_OWNED);
			...
			if(GetHouseValue(h) > GetPlayerMoney(playerid)) ShowInfoBox(playerid, "GarHouse v1.0 By [03]Garsino - Information", "You Can Not Afford To Buy This House.\nHouse Value: $%d.\nYou Have: $%d.\nYou Need: $%d.", GetHouseValue(h), GetPlayerMoney(playerid), (GetHouseValue(h) - GetPlayerMoney(playerid)));
			else
			{
				...
				dini_IntSet(file, "HouseStorage", 0);
				ShowInfoBox(playerid, "GarHouse v1.0 By [03]Garsino - Information", "You've Successfully Bought This House For $%d!", GetHouseValue(h));
				Loop(h2, MAX_HOUSES)
				{
					if(IsHouseInRangeOfHouse(h, h2, RANGE_BETWEEN_HOUSES) && h2 != h)
					{
				...
				DestroyDynamicMapIcon(HouseMIcon[h]);
				UpdateHouseText(h);
				HouseMIcon[h] = CreateDynamicMapIcon(dini_Float(file, "CPOutX"), dini_Float(file, "CPOutY"), dini_Float(file, "CPOutZ"), 32, -1, dini_Int(file, "SpawnWorld"), dini_Int(file, "SpawnInterior"), -1, 50.0);
                #if defined GH_DEBUGGING
...
//------------------------------------------------------------------------------
	if(dialogid == HOUSEMENU+13)
	{
		if(response)
		{
			...
			else
			{
			    if(udb_hash(inputtext) != dini_Int(file, "HousePassword"))
			    {
					...
					ShowInfoBox(playerid, "GarHouse v1.0 By [03]Garsino - Information", "You've Successfully Set The House Password To %s!", inputtext);
                    #if defined GH_DEBUGGING
		...
		{
		    dini_Set(file, "HousePassword", "INVALID_HOUSE_PASSWORD");
			SendClientMessage(playerid, COLOUR_INFO, "The House Password Have Been Removed.");
		}
		return 1;
	}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
		    switch(GetPVarInt(playerid, "HouseIntUpgradeMod"))
		    {
				case 1:
				{
					...
					ShowInfoBox(playerid, "GarHouse v1.0 By [03]Garsino - Information", "You're Now Visiting The House Interior %s.\nThis House Interior Costs $%d.\nYour Visit Time Will End In %d Minute(s).", string, GetPVarInt(playerid, "HousePrevValue"), MAX_VISIT_TIME);
                    #if defined GH_DEBUGGING
				...
				case 2:
				{
					...
					else
					{
						...
						ShowInfoBox(playerid, "GarHouse v1.0 By [03]Garsino - Information", "You've Bought The House Interior %s For $%d.", string, GetPVarInt(playerid, "HousePrevValue"));
                        #if defined GH_DEBUGGING
...
//------------------------------------------------------------------------------
	if(dialogid == HOUSEMENU+16 && response)
	{
		...
		ShowPlayerDialog(playerid, HOUSEMENU+15, DIALOG_STYLE_LIST, "GarHouse v1.0 By [03]Garsino - House Interior", string, "Buy", "Cancel");
		return 1;
	}
//------------------------------------------------------------------------------
//                       House Interior Upgrade
    	...
    	if(listitem == 0) // Deposit
	   	{
     		format(string, sizeof(string), "You Have $%d In Your House Storage.\n\nType In The Amount You Want To Deposit Below:", tmp);
  			ShowPlayerDialog(playerid, HOUSEMENU+11, DIALOG_STYLE_INPUT, "GarHouse v1.0 By [03]Garsino - House Storage", string, "Deposit", "Cancel");
	    }
	    if(listitem == 1) // Withdraw
	    {
     		format(string, sizeof(string), "You Have $%d In Your House Storage.\n\nType In The Amount You Want To Withdraw Below:", tmp);
       		ShowPlayerDialog(playerid, HOUSEMENU+12, DIALOG_STYLE_INPUT, "GarHouse v1.0 By [03]Garsino - House Storage", string, "Withdraw", "Cancel");
    	}
	    if(listitem == 2) // Check Balance
	    {
     		ShowInfoBox(playerid, "GarHouse v1.0 By [03]Garsino - Information", "You Have $%d In Your House Storage.", tmp);
            #if defined GH_DEBUGGING
...
//------------------------------------------------------------------------------
	if(dialogid == HOUSEMENU+11 && response)
	{
		...
		else
		{
			...
			ShowInfoBox(playerid, "GarHouse v1.0 By [03]Garsino - Information", "You've Successfully Deposited $%d Into Your House Storage.\nCurrent Balance: $%d", amount, dini_Int(file, "HouseStorage"));
            #if defined GH_DEBUGGING
...
//------------------------------------------------------------------------------
	if(dialogid == HOUSEMENU+12 && response)
	{
		...
		else
		{
			dini_IntSet(file, "HouseStorage", (dini_Int(file, "HouseStorage") - amount));
			GivePlayerMoney(playerid, amount);
			ShowInfoBox(playerid, "GarHouse v1.0 By [03]Garsino - Information", "You've Successfully Withdrawn $%d From Your House Storage.\nCurrent Balance: $%d", amount, dini_Int(file, "HouseStorage"));
            #if defined GH_DEBUGGING
...
//------------------------------------------------------------------------------
	if(dialogid == HOUSEMENU+60)
	{
		if(response)
		{
            ...
            if(udb_hash(inputtext) != dini_Int(file, "HousePassword"))
			{
				ShowInfoBox(playerid, "GarHouse v1.0 By [03]Garsino - Information", "You've Failed To Enter %s's House Using The Password \"%s\".", GetHouseOwner(GetPVarInt(playerid, "LastHouseCP")), inputtext);
			...
			{
				ShowInfoBox(playerid, "GarHouse v1.0 By [03]Garsino - Information", "You've Successfully Entered %s's House Using The Password \"%s\"!", GetHouseOwner(GetPVarInt(playerid, "LastHouseCP")), inputtext);
				SetPVarInt(playerid, "IsInHouse", 1);
...
dcmd_housemenu(playerid, params[])
{
	...
	{
		ShowPlayerDialog(playerid, HOUSEMENU, DIALOG_STYLE_LIST, "GarHouse v1.0 By [03]Garsino - House Menu", "Sell House\nHouse Storage\nSet House Name\nSet House Password\nBuy House Interior", "Select", "Cancel");
	}
	return 1;
}
//==============================================================================
...
dcmd_createhouse(playerid, params[])
{
	...
	if(h < 0)
	{
		ShowInfoBox(playerid, "GarHouse v1.0 By [03]Garsino - Information", "Sorry, But There Are Already %d Houses Created.\nDelete One Of The Current Ones Or Increase The Limit In The Script.", MAX_HOUSES);
	}
