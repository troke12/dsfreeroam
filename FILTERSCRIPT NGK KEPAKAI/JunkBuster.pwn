...
¦                                                                       ¦
¦ Thanks ~Cueball~ for his zone script, ZeeX for ZCMD				    ¦
¦ and Y_Less for sscanf function.   									¦
¦                                                                       ¦
...
#define JB_VERSION              "10.0"

#define MAX_JB_VARIABLES 		(58)
...
PUBLIC: JB::PutPlayerInVehicle (playerid, vehicleid, seatid)
{
	if (IsPlayerConnected (playerid) && vehicleid != INVALID_VEHICLE_ID)
	{
		...
		{
			JB::PlayerInfo [playerid][JB::pUpdateCount] = GetOptimumUpdateCount (playerid);
		...
		if (SetVehiclePos (vehicleid, x, y, z))
		{
			ForEachPlayer (i)
			{
				if (GetPlayerVehicleID (i) == vehicleid)
				{
					...
					JB::PlayerInfo [i][JB::pUpdateCount] = GetOptimumUpdateCount (i);
				}
			}
			return 1;
		}
...
PUBLIC: JB::TogglePlayerControllable (playerid, toggle)
{
	...
	if (!toggle && IsPlayerInAnyVehicle (playerid))
		JB::PlayerInfo [playerid][JB::pRespraying] = 3;
...
PRIVATE: JB::MutePlayer (playerid, time, reason [])//Time is seconds.
{
	if (IsPlayerConnected (playerid) && time)
	{
		...
		JB::SendFormattedMessageToAll (JB_RED, "JunkBuster:{FFFF00} Muting player '%s' (%d) for %d second(s). Reason: %s", JB::PlayerInfo [playerid][JB::pName], playerid, time, reason);
		return 1;
	}
	return 0;
}
...
PRIVATE: JB::Kick (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		JB::SendFormattedMessageToAll (JB_RED, "JunkBuster:{FFFF00} Kicking player '%s'. Reason: %s", JB::PlayerInfo [playerid][JB::pName], reason);
		JB::LogEx ("%s (%s) has been kicked for %s.", JB::PlayerInfo [playerid][JB::pName], JB::PlayerInfo [playerid][JB::pIp], reason);
...
PRIVATE: JB::Ban (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		...
		TogglePlayerControllable (playerid, false);
		SetCameraBehindPlayer (playerid);
		JB::SendFormattedMessageToAll (JB_RED, "JunkBuster:{FFFF00} Banning player '%s'. Reason: %s", JB::PlayerInfo [playerid][JB::pName], reason);
...
PRIVATE: DisplayKickBanText (playerid, reason [], bool: ban)
{
	if (IsPlayerConnected (playerid))
	{
		...
		format (string, sizeof (string), "~y~Name:~n~~w~%s~n~~y~Date:~n~~w~%s~n~~y~Time:~n~~w~%s~n~~y~IP:~n~~w~%s~n~~y~Reason:~n~~w~%s", JB::PlayerInfo [playerid][JB::pName], JB::GetDate (), JB::GetTime (),
...
PRIVATE: GetOptimumUpdateCount (playerid, base = 20)
{
...
PRIVATE: JunkBusterReport (playerid, report [], details [])
{
	if (IsPlayerConnected (playerid))
	{
		...
		format (string, sizeof (string), "JunkBuster:{FFFF00} Suspected player with %s: %s (%d, %s), %s.", report, JB::PlayerInfo [playerid][JB::pName], playerid, JB::PlayerInfo [playerid][JB::pIp], details);
		ForEachPlayer (i)
...
PUBLIC: JunkBuster ()
{
	...
	ForEachPlayer (i)
	{
		if (DOB::GetBit (JB::FullyConnected, i) && GetPlayerPos (i, x, y, z))
		{
			...
			if (JB::PlayerInfo [i][JB::pRespraying])
			    JB::PlayerInfo [i][JB::pRespraying]--;
			...
			if (!JBGMC::Progress {i})
			{
				...
				if (freezed && JB::Variables [FREEZE_UPDATE])//Prevent that cheaters unfreeze themselves.
				{
					...
					JB::PlayerInfo [i][JB::pRespraying] = 3;
				}
			...
			if (!JB::IsPlayerAdmin (i) && IsPlayerInValidState (i) && !DOB::GetBit (JB::AntiBugKilled, i)) //!JB::PlayerInfo [i][JB::pAntiBugKilled])
			{
				...
				if (JB::Variables [MIN_FPS])
				{
					...
					if (var < JB::Variables [MIN_FPS])
					{
						...
						{
							JB::SendFormattedMessage (i, JB_RED, "JunkBuster:{FFFF00} Please fix your framerate (FPS) or you will get kicked! (Min %d, your FPS: %d)", JB::Variables [MIN_FPS], var);
							SendClientMessage (i, JB_RED, "JunkBuster:{FFFF00} Pressing F7 once may help to fix it. (Removes the outlines of the letters in chat window.)");
						}
					}
				}

				if (!JB::PlayerInfo [i][JB::pUpdateCount])
				{
					...
					if (JB::Variables [AIRBRAKE] && !surfing && (JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_ONFOOT || (JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_DRIVER && JB::GetPlayerSpeed (i, true) < 30)))
					{
					...
					if (JB::PlayerInfo [i][JB::pCurrentState] == PLAYER_STATE_DRIVER)
					{
						...
						if (IsPlayerInPayNSpray (i) || 1 <= GetPlayerInterior (i) <= 3)
						    JB::PlayerInfo [i][JB::pRespraying] = 5;
							...
							if (!IsPlayerInPlane (i))
							{
								...
								if (JB::Variables [WALLRIDE])
								{
								    ...
								    if (75.0 < abs (rz) < 105.0)
								    {
										...
										if (floatround (rz * 160.0) < JB::Variables [WALLRIDE] && ivar [3] > 10)
										{
				...
				if (JB::Variables [WEAPON_HACK] && !JB::PlayerInfo [i][JB::pUpdateCount])
				{
				...
				if (JB::Variables [SS_HEALTH] && !JB::PlayerInfo [i][JB::pUpdateCount])
				{
					if (health > JB::PlayerInfo [i][JB::pHealth] && !JB::PlayerInfo [i][JB::pVendingMachineUsed])
					{
					...
					else if (armour > JB::PlayerInfo [i][JB::pArmour])
					{
						JB::Warnings [i]{SS_HEALTH}++;
						if (JB::Warnings [i]{SS_HEALTH} >= MAX_CHECKS)
	...
	PUBLIC: TempBan (playerid, days, reason [])
	{
		if (days > 0)
		{
		    if (TempBanCount < sizeof (TempBanInfo))
		    {
				...
				TempBanInfo [TempBanCount][tbIp] = JB::PlayerInfo [playerid][JB::pIp];
				TempBanInfo [TempBanCount][tbName] = JB::PlayerInfo [playerid][JB::pName];
				++TempBanCount;
	...
	PUBLIC: DeleteTempBan (name [])
	{
		for (new i = 0; i < TempBanCount; ++i)
		{
			if (!strcmp (TempBanInfo [i][tbName], name, false))
			{
				...
				TempBanInfo [i][tbName] = TempBanInfo [TempBanCount][tbName];
				TempBanInfo [i][tbIp] = TempBanInfo [TempBanCount][tbIp];
				SaveTempBans ();
				return 1;
			}
		}
...
PRIVATE: ConfigJunkBuster ()
{
	...
	{
		JB::Homepage [0] = '\0';
		strcat (JB::Homepage, DOF2::GetString (CONFIG_FILE, "Homepage", "Misc"));
	}
...
{
	JB::PlayerInfo [playerid][JB::pName][0] = '\0';
	strcat (JB::PlayerInfo [playerid][JB::pName], name, MAX_PLAYER_NAME);
	return 1;
}
...
CALLBACK: OnPlayerUpdate (playerid)
{
	...
	if (JB::PlayerInfo [playerid][JB::pUpdateCount])
		JB::PlayerInfo [playerid][JB::pUpdateCount]--;
	else
	{
		GetPlayerHealth (playerid, fvar);
		if (fvar < JB::PlayerInfo [playerid][JB::pHealth])
			JB::PlayerInfo [playerid][JB::pHealth] = fvar;

		GetPlayerArmour (playerid, fvar);
		if (fvar < JB::PlayerInfo [playerid][JB::pArmour])
	...
	if (ivar == JB::PlayerInfo [playerid][JB::pCurrentState] && ivar == PLAYER_STATE_DRIVER)
	{
	    ...
	    if (JB::PlayerInfo [playerid][JB::pLastVehicle] != 0 && vehicleid != 0 && !JB::IsPlayerAdmin (playerid))
	    {
		    ...
		    else if (JB::Variables [VEHICLE_REPAIR] && !JB::PlayerInfo [playerid][JB::pRespraying])
		    {
			 	...
			 	if (JB::Variables [ARMED_VEHICLES])
			 	{
					...
					if (IsArmedVehicle (vehicleid))
					{
					    if (++JB::Warnings [killerid]{ARMED_VEHICLES} >= JB::Variables [ARMED_VEHICLES])
						{
							...
							JB::SendFormattedMessage (killerid, JB_RED, "JunkBuster:{FFFF00} Killing with a %s is not allowed! Stop it! (Warning %d/%d)", JB::GetVehicleName (vehicleid), JB::Warnings [killerid]{ARMED_VEHICLES}, JB::Variables [ARMED_VEHICLES]);
					}
			 	}
			}
...
CALLBACK: OnPlayerConnect (playerid)
{
	...
	JB::PlayerInfo [playerid][JB::pUpdateCount] = 0;
	...
	{
		SendClientMessage (playerid, JB_GREEN_BLUE, "> This server is running {FF0000}JunkBuster Anti-Cheat{00D799}!");
...
CALLBACK: OnPlayerText (playerid, text [])
{
	...
	{
		JB::SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} You are not allowed to chat for %d second(s) because you have been muted!", JB::PlayerInfo [playerid][JB::pMuted]);
		return 0;
	}
	...
	if (!JB::IsPlayerAdmin (playerid) || !JB::Variables [ADMIN_IMMUNITY])
	{
        ...
        JB::PlayerInfo [playerid][JB::pLastMessage][0] = '\0';
		strcat (JB::PlayerInfo [playerid][JB::pLastMessage], text, 128);
...
COMMAND:tban (playerid, params [])
{
	...
	if (JB::sscanf (params, "iiz", id, days, reason))
		return SendClientMessage (playerid, JB_RED, "Usage:{D6D6D6} /tban <ID> <days> <reason>");
...
{
	if (JB::Variables [SPECTATE_HACK] && newstate == PLAYER_STATE_SPECTATING && !JB::IsPlayerAdmin (playerid) && !IsPlayerNPC (playerid) && !DOB::GetBit (JB::Spectating, playerid))
...
CALLBACK: OnPlayerKeyStateChange (playerid, newkeys, oldkeys)
{
	...
	if (!JB::IsPlayerAdmin (playerid) || !JB::Variables [ADMIN_IMMUNITY])
	{
		...
		if (JB::Variables [JOYPAD] && ((newkeys & KEY_ANALOG_UP) || (newkeys & KEY_ANALOG_DOWN) || (newkeys & KEY_ANALOG_LEFT) || (newkeys & KEY_ANALOG_RIGHT)))
		{
		    if (++JB::Warnings [playerid]{JOYPAD} >= JB::Variables [JOYPAD])
		        JB::Kick (playerid, "Using joypad");
			else
			{
			    JB::PlayerInfo [playerid][JB::pFreezeTime] = 5; // Freeze for 5 seconds.
			    JB::PlayerInfo [playerid][JB::pRespraying] = 3;
				TogglePlayerControllable (playerid, false);
				SendClientMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Please stop using a joypad/gamepad. Use your keyboard and mouse.");
			}
		}
	}
...
CALLBACK: OnPlayerEnterRaceCheckpoint (playerid)
{
	...
	if (!JB::PlayerInfo [playerid][JB::pUpdateCount] && !JB::IsPlayerAdmin (playerid))
	{
