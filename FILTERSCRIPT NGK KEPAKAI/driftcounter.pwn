...
public Drift(){ // By Luby
        ...
        for(new g=0;g<200;g++){
                ...
                if(IsPlayerInAnyVehicle(g) && GetVType(GetPlayerVehicleID(g)) && floatabs(floatsub(Angle1, Angle2)) > DRIFT_MINKAT && floatabs(floatsub(Angle1, Angle2)) < DRIFT_MAXKAT && BySpeed > DRIFT_SPEED){
                        ...
                        PlayerDriftCancellation[g] = SetTimerEx("DriftExit", 2000, 0, "d", g);
                }
                ...
                if(DriftPointsNow[g] > 70 && DriftPointsNow[g]<10000){
					...
					TextDrawHideForPlayer(g,TDLabels[0]);
					TextDrawHideForPlayer(g,TDLabels[1]);
					TextDrawHideForPlayer(g,TDLabels[2]);

	                TextDrawHideForPlayer(g,TDValueDrift[g]);
	                TextDrawHideForPlayer(g,TDValueBonus[g]);
	                TextDrawHideForPlayer(g,TDValueCash[g]);
	...
	for(i=0;i<=MAX_PLAYERS;i++){
	    ...
	    if(DriftMode[i] && cs==PLAYER_STATE_DRIVER && DriftPointsNow[i]>70){
	        ...
	        if(h<HealthInit[i]){
	        	...
	        	DriftExit(i);
	        	GameTextForPlayer(i,"~n~~n~~n~~n~~n~~n~~n~~r~Crash",800,5);
	        	DriftMode[i]=false;
			}
		}
	    if(cs==PLAYER_STATE_DRIVER && DriftMode[i]==false){
	        if(GetVType(GetPlayerVehicleID(i))){
		        ...
		        AutoFixBool[i]=false;
		        DriftTimer[i]=SetTimerEx("Drift", 100, true, "i", i);
			}
		}
