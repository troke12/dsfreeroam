...
public OnFilterScriptInit()
{
	...
	print("*EBank Loaded*");
	print("**************\n");
	GetinBank = CreatePickup(1318, 1, 595.800048, -1251.503295, 18.324462, 0);
...
{
    SetPlayerMapIcon(playerid, 12, 595.800048, -1251.503295, 18.324462, 52, 1);
...
public OnPlayerPickUpPickup(playerid,pickupid)
{
	...
	{
		SetPlayerPos(playerid,2308,-16, 27);
		SendClientMessage(playerid,COLOR_YELLOW,"Type /bank");
		SetCameraBehindPlayer(playerid);
		return 0;
	}
	if(pickupid == GetoutBank)
	{
		SetPlayerPos(playerid,594.151428,-1245.515747, 18.105913);
...
{
	if(dialogid == 440) //deposit
    {
		new file[100],Name[MAX_PLAYER_NAME]; GetPlayerName(playerid,Name,sizeof(Name)); format(file,sizeof(file),PlayerFile,Name);
 		if(!response) return ShowPlayerDialog(playerid,445,DIALOG_STYLE_LIST,"Bank","Deposit\nWithdraw\nBalance\nTransfer","Select","Cancel");
   		...
   		else if(!IsNumeric(inputtext))
		{
			new string[128];
 			...
 			ShowPlayerDialog(playerid,440,DIALOG_STYLE_INPUT,"Deposit",string,"Deposit","Back");
		...
		else
		{
			...
			ShowPlayerDialog(playerid,445,DIALOG_STYLE_LIST,"Bank","Deposit\nWithdraw\nBalance\nTransfer","Select","Cancel");
		}
    }
	if(dialogid == 443) //withdraw
	{
  		new file[100],Name[MAX_PLAYER_NAME]; GetPlayerName(playerid,Name,sizeof(Name)); format(file,sizeof(file),PlayerFile,Name);
     	if(!response) return ShowPlayerDialog(playerid,445,DIALOG_STYLE_LIST,"Bank","Deposit\nWithdraw\nBalance\nTransfer","Select","Cancel");
        ...
        else if(!IsNumeric(inputtext))
        {
        	...
        	ShowPlayerDialog(playerid,443,DIALOG_STYLE_INPUT,"Withdraw",string,"Withdraw","Back");
        	SendClientMessage(playerid,COLOR_RED,"Please Use Numbers");
        }
		else
		{
    ...
    if(dialogid == 450) //transfer (choose playerid)
	{
     	if(!response) return ShowPlayerDialog(playerid,445,DIALOG_STYLE_LIST,"Bank","Deposit\nWithdraw\nBalance\nTransfer","Select","Cancel");
        ...
        else if(!IsNumeric(inputtext))
        {
	    	...
	    	ShowPlayerDialog(playerid,450,DIALOG_STYLE_INPUT,"Transfer",string,"Next","Back");
		...
		else
		{
			...
			ShowPlayerDialog(playerid,451,DIALOG_STYLE_INPUT,"Transfer",string,"Transfer","Back");
		}
	}
	if(dialogid == 451) //transfer (choose amount)
	{
  		new file[100],Name[MAX_PLAYER_NAME]; GetPlayerName(playerid,Name,sizeof(Name)); format(file,sizeof(file),PlayerFile,Name);
     	if(!response) return ShowPlayerDialog(playerid,445,DIALOG_STYLE_LIST,"Bank","Deposit\nWithdraw\nBalance\nTransfer","Select","Cancel");
			...
			case 2:
     		{
				...
				ShowPlayerDialog(playerid,444,DIALOG_STYLE_MSGBOX,"Balance",string,"Ok","Back");
			}
			case 3:
     		{
				new string[128];
